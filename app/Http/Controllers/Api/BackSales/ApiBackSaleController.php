...
namespace App\Http\Controllers\Api\BackSales;
...
class ApiBackSaleController extends Controller
{
    public function __construct()
    {
        $this->middleware(['auth:api','apiPermission:backSalesDepartment'])
            ->only([
                'makeSaleOrder',
                'saleOrders',
                'single_sale_order',
    ...
    public function makeSaleOrder(Request $request)
    {
        ...
        $request->validate([
            ...
            'discount_value'=>'nullable|regex:/^\d+(\.\d{1,2,0})?$/',
        ...
        return response()->json(['message'=>'Back Sale Order Has Created'],200);
...
//            200);
    ...
    public function single_sale_order(Request $request)
    {
        $this->validate($request,[
            'back_sale_id'=>'required|exists:sales,id'
        ]);
        $row =  Sale::where([
           'user_id'=>$request->user()->trader_id,
            'sale_type'=>'back_sale',
           'id'=>$request->back_sale_id
       ])->with([
            'trader',
            'client',
            'coupon',
    ...
    public function deleteSaleOrder(Request $request)
    {
        ...
        Sale::destroy($request->back_sale_id);
